...
use voidrun_simulation::components::player_shooting::{AimMode, ToggleADSIntent, ease_out_cubic};
use crate::systems::{VisualRegistry, SceneRoot, AttachmentRegistry, GodotDeltaTime};
...
pub fn calculate_ads_target_transform_cameraline(
    ...
    let Some(camera_pivot_node) = player_actor_node.get_node_or_null("%CameraPivot") else {
        voidrun_simulation::log_error("❌ CameraPivot не найден (нужен unique name в player prefab)");
        return None;
    };
    ...
    let Some(camera_line) = player_actor_node.get_node_or_null("%CameraLine") else {
        voidrun_simulation::log_error("❌ CameraLine не найден (нужен unique name в player prefab)");
        return None;
    };
    ...
    let Some(sight_socket_node) = weapon_node.get_node_or_null("%SightSocket") else {
        voidrun_simulation::log_error("❌ SightSocket не найден в weapon (нужен unique name)");
        return None;
    };
        ...
        match aim_mode.as_ref() {
            AimMode::HipFire => {
                ...
                *aim_mode = AimMode::EnteringADS {
                ...
                voidrun_simulation::log("▶️ Entering ADS");
            }
            ...
            AimMode::ADS => {
                ...
                *aim_mode = AimMode::ExitingADS {
                ...
                voidrun_simulation::log("◀️ Exiting ADS");
            }

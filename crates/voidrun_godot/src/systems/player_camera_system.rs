...
use godot::prelude::*;
use voidrun_simulation::components::{ActiveCamera, CameraMode, Player};
use voidrun_simulation::PrefabPath;

use crate::input::{CameraToggleEvent, MouseLookEvent};
use crate::systems::{SceneRoot, VisualRegistry};
...
pub fn setup_player_camera(
    ...
    for player_entity in player_query.iter() {
        ...
        let Some(mut camera_pivot) = player_node.try_get_node_as::<godot::classes::Node3D>("%CameraPivot") else {
            voidrun_simulation::log_error("❌ CameraPivot not found in test_player.tscn! Check scene structure.");
            continue;
        };
        ...
        voidrun_simulation::log("✅ Player FPS camera ready (V to toggle, mouse to look)");
    }
}
        ...
        match new_mode {
            CameraMode::FirstPerson => {
                ...
                let Some(mut player_camera) = player_node.try_get_node_as::<Camera3D>("%CameraPivot/PlayerCamera") else {
                    voidrun_simulation::log_error("❌ PlayerCamera not found!");
                    continue;
                };
                ...
                voidrun_simulation::log("📷 First-Person Camera");
            }
            ...
            CameraMode::RTS => {
                ...
                let Some(mut rts_camera) = scene_root
                    ...
                    voidrun_simulation::log_error("❌ RTS Camera not found in scene!");
                    continue;
                };
                ...
                voidrun_simulation::log("📷 RTS Camera (strategic view)");
            }
        }
    }
}
